...
`);
  ...
  function create(win, registry, name2 = "HTMLCLI") {
    ...
    cli.run = async (paramString = null) => {
      const window2 = win;
      const { HTMLCLI, location, URLSearchParams: URLSearchParams2 } = window2;
      ...
      if (stdin) {
        ...
        replaced.prompt = window2.prompt;
        const buffer = textStr.trim().split(/\n\r?/g);
        window2.prompt = () => buffer && buffer.length > 0 ? buffer.shift() : null;
      }
      const out = params.get("out") || params.get("append");
      if (out) {
        replaced.console = window2.console;
...
`);
        window2.console = { log, error: log, info: log, group: log, endgroup: log };
      }
      if (stdin || out) {
        HTMLCLI.registry.utils._globalBundleStdLibCommands(window2, window2.document);
      }
      try {
      ...
      if (Object.keys(replaced).length) {
        if (replaced.console) {
          window2.console = replaced.console;
        }
        if (replaced.prompt) {
          window2.prompt = replaced.prompt;
        }
        HTMLCLI.registry.utils._globalBundleStdLibCommands(window2, window2.document);
      }
    };
...
`) : null;
  ...
  // scripts/window-export.mjs
  window.oludom = { Browser, BrowserPage, WindowFrame };
})();
